...
public class CommandRemoveTrigger extends Command {

	public CommandRemoveTrigger(String commandString, String helpString) {
	...
	public void execute(IChannel channel, IUser commandAuthor, IGuild guild, List<String> command, IMessage message) {
		if(Util.hasPermission(PermissionLevels.PERMISSION_LEVEL_TRUSTED, guild, commandAuthor)){
			if(command.size() != 4){
				...
				if(currentTriggers.contains(command.get(3).toLowerCase())){
					currentTriggers.remove(command.get(3).toLowerCase());
				}
				...
				if(!indexFileContents.isEmpty() && (audio || image)){
					...
					if(image){
						...
						channel.sendMessage("Successfully removed the trigger `" + command.get(3) + "` from the image `" + command.get(2) + "`");
						return;
					}else{
						Util.saveJsonToFile(Util.getAudioIndexFile(guild), indexFileContents);
						channel.sendMessage("Successfully removed the trigger `" + command.get(3) + "` from the audio clip `" + command.get(2) + "`");
						return;
					}
				}else{
